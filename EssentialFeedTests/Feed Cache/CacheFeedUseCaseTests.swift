...
class LocalFeedLoader {
    ...
    func save(_ items: [FeedItem], completion: @escaping (Error?) -> Void) {
        store.deleteFeedCache { [weak self] error in
            ...
            if error == nil {
                store.insert(items, timesamp: currentDate(), completion: completion)
            } else {
                completion(error)
            }
        }
...
final class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_doesNotDeliverDeletionErrorAfterSUTInstanceHasBeenDeallocated() {
        ...
        sut?.save([uniqueItem()]) { result in receivedResults.append(result) }
        sut = nil
    ...
    private class FeedStoreSpy: FeedStore {
        ...
        func insert(_ items: [FeedItem], timesamp: Date, completion: @escaping InsertionCompletion) {
            ...
            receivedMessages.append(.insert(items, timesamp))
        }
    }
